...
bool valid_arguments(double gamma, double eta_poly, exd::physics::ModelStatus& status)
{
    if (gamma <= 1.0)
    {
        status.ok = false;
...
bool valid_pi(double pi, exd::physics::ModelStatus& status)
{
    if (pi <= 0.0)
    {
        status.ok = false;
...
bool valid_tau(double tau, exd::physics::ModelStatus& status)
{
    if (tau <= 0.0)
    {
        status.ok = false;
