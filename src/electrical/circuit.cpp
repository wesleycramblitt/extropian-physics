...
// (mechanics::IMomentModel). Concrete model classes are hidden in an
// anonymous namespace and reachable only through DcMotorModel /
// make_dc_motor_moment.

#include <exd/physics/electrical/circuit.hpp>
...
double DcMotorModel::step(double dt, double omega, exd::physics::ModelStatus& status)
{
    if (dt <= 0.0)
    {
        status.ok = false;
...
class DcMotorMomentModel final : public mechanics::IMomentModel
{
public:
    ...
    double moment(const mechanics::RotationalState& state,
