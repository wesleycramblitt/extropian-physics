...
#include <exd/physics/electrical/static_fields.hpp>
...
StaticFieldResult solve_static_field(const StaticFieldConfig& config)
{
    ...
    result.potential = coupling::StructuredScalarGrid{origin, config.spacing,
                                                      config.dims, std::move(phi)};
    result.field_vector = coupling::StructuredVectorGrid{origin, config.spacing,
