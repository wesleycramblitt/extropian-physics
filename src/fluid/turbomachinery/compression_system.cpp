                                                    ...
                                                    exd::physics::ModelStatus& status)
{
    ...
    std::unique_ptr<thermo::IEos> eos = thermo::make_ideal_gas(thermo::IdealGasConfig{});
    if (!eos)
    {
        result.valid = false;
    ...
    electrical::DcMotorModel motor(config.motor);
    std::unique_ptr<control::IController> governor;
    if (config.governor_enabled)
    {
        governor = control::make_pi_controller(config.governor_pi);
        if (!governor)
        {
            result.valid = false;
    ...
    solver::IntegratorConfig integration;
    integration.method = solver::IntegrationMethod::RK4;
    ...
    for (uint64_t step = 0; step < config.max_steps; ++step)
    {
        ...
        ModelStatus ist;
        const bool iok = solver::integrate_step(integration, t, config.dt,
