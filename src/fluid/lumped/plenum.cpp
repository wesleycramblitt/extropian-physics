...
namespace exd::physics::fluid::lumped {
                                   ...
                                   exd::physics::ModelStatus& status)
{
    ...
    {
        out.status = exd::physics::ModelStatus{false, verror, out.status.warnings};
        status = out.status;
        return out;
    }
    ...
    if (!std::isfinite(state.p_plenum) || state.p_plenum <= 0.0)
    {
        ...
        out.status = exd::physics::ModelStatus{false, err, out.status.warnings};
        status = out.status;
        return out;
    }
    ...
    out.ok = true;
    out.status = exd::physics::ModelStatus{true, "", out.status.warnings};
    status = out.status;
    return out;
}
...
PlenumState step_plenum(double dt,
                        ...
                        const PlenumCharacteristic& throttle,
                        const thermo::IEos& eos,
                        const solver::IntegratorConfig& integration,
                        exd::physics::ModelStatus& status)
{
    if (dt <= 0.0)
    {
    ...
    exd::physics::ModelStatus substatus;
    ...
    const bool ok = solver::integrate_step(integration, 0.0, dt,
