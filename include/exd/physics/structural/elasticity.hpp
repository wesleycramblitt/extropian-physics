...
namespace exd::physics::structural {
...
struct ElasticityResult
{
    ...
    coupling::StructuredVectorGrid displacement; // 3*N flat, values[3*idx + c], idx = i + nx*(j + ny*k)
...
ElasticityResult solve_elasticity(const ElasticityConfig& config,
                                  ModelStatus& status,
                                  const coupling::IScalarField3D* temperature_channel = nullptr);
