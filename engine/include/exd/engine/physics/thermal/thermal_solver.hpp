...
// exd::engine::coupling::StructuredScalarGrid (same origin/spacing/dims
...
#include <exd/engine/coupling/field_channels.hpp>   // StructuredScalarGrid (result payload only)
...
struct ThermalConfig {
    ...
    const exd::engine::coupling::IVectorField3D* velocity_channel = nullptr;
...
struct ThermalResult {
    bool ok = false;
    ModelStatus status;
    exd::engine::coupling::StructuredScalarGrid temperature;  // node-centered, same grid as config
...
struct ThermalState {
    double time = 0.0;                          // s
    exd::engine::coupling::StructuredScalarGrid temperature; // node-centered
