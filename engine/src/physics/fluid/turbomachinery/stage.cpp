...
StageResult solve_stage(const StageConfig& config, const StageInlet& inlet,
                        double omega, double mdot, const exd::engine::physics::thermo::IEos& eos,
                        exd::engine::core::ModelStatus& status)
{
    ...
    double pi = 0.0;
    if (delta_h0 > 0.0)
        pi = exd::engine::physics::thermo::polytropic::pressure_ratio_compression(tau, gamma, eta_poly, status);
    else
        pi = exd::engine::physics::thermo::polytropic::pressure_ratio_expansion(tau, gamma, eta_poly, status);
    if (!status.ok)
    {
        result.status = status;
        return result;
