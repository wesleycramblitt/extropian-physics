...
// exd::engine::numerics::integrate_step() integrator.
...
namespace exd::engine::physics::reaction {
...
ChemistryResult solve_chemistry(const ChemistryConfig& config, ModelStatus& status)
{
    ...
    for (std::uint64_t step = 0; step < total_steps; ++step)
    {
        ...
        double dt_used = 0.0;
        const bool ok = exd::engine::numerics::integrate_step(config.integration, t, step_dt,
