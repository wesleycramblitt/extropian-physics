...
void smear_point_force(std::vector<double>& fx, std::vector<double>& fy,
                       std::vector<double>& fz, const exd::engine::physics::fluid::fdm3::FDM3Config& flow,
...
std::array<double, 3> fluid_momentum(const exd::engine::physics::fluid::fdm3::FDM3Solver& solver)
{
    const auto& f = solver.field();
    const exd::engine::physics::fluid::fdm3::FDM3Config& cfg = solver.config();
...
void apply_smeared_point_force(std::vector<double>& fx, std::vector<double>& fy,
                               ...
                               const exd::engine::physics::fluid::fdm3::FDM3Config& flow,
                               const std::array<double, 3>& position,
...
DragBodyResult simulate_drag_body(const DragBodyConfig& config, ModelStatus& status)
{
    ...
    // ── Setup ───────────────────────────────────────────────────────────
    exd::engine::physics::fluid::fdm3::FDM3Solver solver;
    if (!solver.initialize(config.flow, status))
        return fail(status, result, "drag body: " + status.error), result;

    exd::engine::physics::rigid_body::RigidBodyConfig rb_cfg;
    rb_cfg.mass = config.mass;
    rb_cfg.inertia_principal = config.inertia_principal;
    auto dynamics = exd::engine::physics::rigid_body::make_rigid_body_dynamics(
        exd::engine::physics::rigid_body::RigidBodyIntegration::SymplecticEuler, rb_cfg);

    exd::engine::physics::rigid_body::RigidBodyState body;
    ...
    for (uint64_t exchange = 0; exchange < config.max_steps; ++exchange)
    {
        ...
        exd::engine::physics::rigid_body::RigidBodyForces loads;
        for (int c = 0; c < 3; ++c)
        {
