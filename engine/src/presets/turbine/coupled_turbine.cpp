...
using exd::engine::physics::fluid::fdm3::BoundaryFace;
...
CoupledTurbineResult run_coupled_turbine(const CoupledTurbineConfig& cfg,
                                         ModelStatus& status)
{
    using exd::engine::physics::fluid::forces::BladeGeometry;
    using exd::engine::physics::fluid::forces::BladeStation;
    using exd::engine::physics::fluid::forces::ForceEvaluatorType;
    using exd::engine::physics::fluid::forces::SurfaceFlow;
    using exd::engine::physics::rigid_body::MomentResult;
    using exd::engine::physics::rigid_body::RotationalState;
    ...
    const double oy = cfg.rotor_origin[1];
    const double oz = cfg.rotor_origin[2];
    const exd::engine::physics::rigid_body::RotationAxis axis{{ox, oy, oz - blade.z_rotor}, {0.0, 0.0, 1.0}};

    // Force evaluator: local blade element, created ONCE and reused across
    // exchanges (no per-exchange evaluator allocation).
    exd::engine::physics::fluid::forces::PolarDatabase polars;
    polars.add_builtin_polars();
    exd::engine::physics::fluid::forces::ForceEvaluatorParams fparams;
    ...
    std::unique_ptr<exd::engine::physics::fluid::forces::IForceEvaluator> evaluator =
        exd::engine::physics::fluid::forces::make_force_evaluator(fparams, status);
    ...
    if (!cfg.generator.omega_pts.empty())
        load = exd::engine::physics::rigid_body::make_curve_moment(cfg.generator);
    std::unique_ptr<exd::engine::physics::rigid_body::IRotationalDynamics> dynamics =
        exd::engine::physics::rigid_body::make_rigid_rotor_dynamics(
            {cfg.rotor_inertia, exd::engine::physics::rigid_body::RotationalIntegration::Heun});
    ...
    for (uint64_t step = 0; step < cfg.max_steps; ++step)
    {
        ...
        if (step % cfg.fluid_steps_per_exchange == 0)
        {
            ...
            std::unique_ptr<exd::engine::coupling::IFlowField3D> adapter =
                exd::engine::coupling::make_fdm3_field_adapter(solver);
            ...
            for (std::size_t e = 0; e < n_stations; ++e)
            {
                ...
                const MomentResult m = exd::engine::physics::rigid_body::integrate_moment(
                    std::span<const exd::engine::physics::rigid_body::ElementForce3D>(
