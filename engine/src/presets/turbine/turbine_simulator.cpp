...
// generic exd::engine::physics::fluid::forces + mechanics modules and wires them into a coupled
...
#include <exd/engine/presets/turbine/turbine_simulator.hpp>
...
{
    exd::engine::physics::fluid::forces::BladeGeometry geo;
    if (element_count < 4)
    {
    ...
    for (int i = 0; i < element_count; ++i)
    {
        ...
        exd::engine::physics::fluid::forces::BladeStation st;
        st.r = r;
        st.dr = dr;
        st.chord = chord;
...
exd::engine::physics::rigid_body::CurveMomentConfig make_generator_curve(double rated_power,
                                                  ...
                                                  int points)
{
    exd::engine::physics::rigid_body::CurveMomentConfig curve;
                               ...
                               exd::engine::physics::control::IController* governor)
{
    ...
    std::vector<std::string> warnings;
    const exd::engine::physics::fluid::forces::BladeGeometry blade =
    ...
    const exd::engine::physics::fluid::forces::BladeSurface surface =
        exd::engine::physics::fluid::forces::build_blade_surface(blade, state.angle_rad, axis);
    ...
    const exd::engine::physics::fluid::forces::SurfaceFlow sampled =
        exd::engine::coupling::sample_flow(flow, surface, /*p_ref*/ 0.0);

    std::unique_ptr<exd::engine::physics::fluid::forces::IForceEvaluator> evaluator =
        exd::engine::physics::fluid::forces::make_force_evaluator(config.force, status);
    if (!evaluator)
    {
        status.ok = false;
    ...
    const exd::engine::physics::rigid_body::MomentResult moments = exd::engine::physics::rigid_body::integrate_moment(per_element, axis);
    if (!moments.valid)
    {
        status.ok = false;
    ...
    std::unique_ptr<exd::engine::physics::rigid_body::IMomentModel> load =
        exd::engine::physics::rigid_body::make_curve_moment(config.generator);
    ...
    std::unique_ptr<exd::engine::physics::rigid_body::IRotationalDynamics> dynamics =
        exd::engine::physics::rigid_body::make_rigid_rotor_dynamics({config.inertia, config.integration});
    const exd::engine::physics::rigid_body::RotationalState new_state =
                                  ...
                                  const TurbineConfig& config)
{
    ...
    exd::engine::coupling::UniformFieldConfig field_config;
    ...
    std::unique_ptr<exd::engine::coupling::IFlowField3D> field =
        exd::engine::coupling::make_uniform_field(field_config);
    ...
    exd::engine::physics::rigid_body::RotationalState state{config.initial_omega, 0.0};
