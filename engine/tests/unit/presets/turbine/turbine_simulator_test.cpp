...
using exd::engine::physics::fluid::forces::Freestream;
using exd::engine::physics::rigid_body::ModelStatus;
...
std::unique_ptr<exd::engine::coupling::IFlowField3D> uniform_field(const Freestream& fs)
{
    return exd::engine::coupling::make_uniform_field(
        exd::engine::coupling::UniformFieldConfig{fs.velocity, fs.rho, fs.mu, fs.p_ref});
}
...
TEST_CASE("make_generator_curve: moment model clamps flat below min_omega")
{
    ...
    auto model = exd::engine::physics::rigid_body::make_curve_moment(curve);
