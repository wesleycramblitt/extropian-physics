...
std::array<double, 3> node_at(const exd::engine::coupling::StructuredVectorGrid& g,
                              int i, int j, int k)
{
    const int nx = g.dims[0], ny = g.dims[1];
    const size_t idx = static_cast<size_t>(i) + static_cast<size_t>(nx) *
...
{
    struct TempField : exd::engine::coupling::IScalarField3D
    {
        double T_ref = 300.0;
        double gz = 1.0; // K/m
...
TEST_CASE("Elasticity: channel adapter smoke")
{
    ...
    auto channel = exd::engine::coupling::make_vector_grid_field(res.displacement);
